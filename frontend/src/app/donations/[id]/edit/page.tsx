...
export default function EditDonationPage() {
  ...
  const [form, setForm] = useState<DonationFormState | null>(null);
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const message = err instanceof Error ? getErrorMessage(err, "Impossible de charger ce don.") : "Impossible de charger ce don.";
  ...
  function buildPayload(): CreateDonationPayload {
    if (!form) throw new Error("Formulaire non chargé.");
    return {
  ...
  async function handleSubmit(e: SubmitEvent<HTMLFormElement>) {
    ...
    setSubmitting(true);
    try {
      await api.updateDonation(id, buildPayload());
